...
#define strncpy_ex(a,b,c)  {strncpy(a,b,c);a[c-1]=0;}   //M-LNX-119
...
SETCOMMAND command_array[MAXSCA];
...
int mk_basedir(char *argv0){
  char *p;
  int ok = 0;

  p = realpath(argv0 , BASEDIR);
  if (p == NULL){
  ...
  if(p){
    *p = 0;                        //$/opt/XXX/Printer/ENV/MODEL/cupswrapper
    p = strrchr(BASEDIR , '/');
    if(p){
      *(p+1)=0;                    //$/opt/XXX/Printer/ENV/MODEL/
      ok = 1;
    }
    else{
      strcat(p , "/../");          // $/opt/XXX/Printer/ENV/MODEL/cupswrapper/../
      ok = 1;
    }
  }
  else{
...
int main(int argc,char * argv[]) {
  char *printer;
  char ppd_line[500],tmp[500],*p_tmp,tmp_n[32],tmp_op[500];
  FILE *fp_ppd;
  char *p;
  ...
  for ( i = 0; default_setting[i] != NULL; i ++){
    add_command_list_brcommand(default_setting[i]);
  }
  ...
  while(fgets(ppd_line,sizeof(ppd_line),fp_ppd)){
    ...
    for ( i = 0; PPDdefaultN[i].option!= NULL; i ++){
      strncpy_ex(tmp,PPDdefaultN[i].option, sizeof(tmp) );
      p_tmp = tmp;
      if(tmp[0] == '^')p_tmp ++;
  ...
  //************************************
  {
    ...
    DEBUGPRINT("main:set brother command line option (option described in PPD)\n");
    write_log_file(5,"BROTHER COMMAND LINE SETTING(PPD)\n");  
    for ( i = 0; ppdcommand_all_list[i]!= NULL; i ++){
      strncpy_ex(label,
	...
	strncpy_ex(str,ppdcommand_all_list[i]->ppdcommandlist[ii].value,
...
int exec_brprintconf(char *brprintconf,char *printer){
  ...
  for ( i = 0; (command_array[i].value[0] != 0)&&(i<MAXSCA); i ++ ){
    sprintf(exec,"%s/lpd/%s   -P %s   %s  %s",
	    BASEDIR,
	    brprintconf,printer,
...
int add_command_list(char *option,char  *value){
  ...
  for ( i = 0; (command_array[i].option[0] != 0)&&(i<MAXSCA); i ++ ){
    ...
    if(p){
      ...
      strncpy_ex(command_array[i].value, value, MAXSCV);
      break;
    }
  }
  ...
  if(command_array[i].option[0] == 0){
    ...
    strncpy_ex( command_array[i].option ,option , MAXSCO);
    strncpy_ex( command_array[i].value  ,value , MAXSCV);
...
int add_command_list_brcommand_sub(char  *command){
  ...
  strncpy_ex(option,command,sizeof(option)-1);
  ...
  *p1 = 0;
  ...
  strncpy_ex(value , p1 , sizeof(value));
...
int add_command_list_brcommand(char  *command){
  ...
  strncpy_ex(multi_brcommands,command , sizeof(multi_brcommands));
...
char *strstr_ex(char *data , char *search_data){
  ...
  l = strlen(search_data);
  
  p = data;
  while(1){
    p = strstr(p,search_data);
    if (p == NULL)break;
...
int divide_media_token(char *input,char output[5][30]){
  char media_command[256];
  char *p,*pp;
  int i;

  p = strstr(input,MEDIAEQ);
  if(p){
    strncpy_ex(media_command,p+strlen(MEDIAEQ), sizeof(media_command));
  }
  else{
    return 0;
  }
